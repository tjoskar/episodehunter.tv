function HistoryTvModel(_history) {
    ...
    this.title = _history.series_name;
    this.url = '/tv/' + _history.tv_id + '/' + EH.urlTitle(this.title) + '/' + _history.episode_id;
    this.image = EH.url.series.episode + EH.url.defaultImage.episode;
    ...
    if (_history.image) {
        this.image = EH.url.series.episode + _history.image;
    }
...
function HistoryMovieModel(_history) {
    if (!(this instanceof HistoryMovieModel)) {
        return null;
    }
...
HistoryMovieModel.prototype.extraInfo = function() {
    return '';
};
